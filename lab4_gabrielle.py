...
def draw_special(t, size, repeat, gap):
    for _ in range(repeat):
        draw_circles(t, size, gap)
        t.right(360 / repeat) 
...
def draw_picture_nice():
    ...
    ollie.speed(0)
    for i in range(len(colors)):
        ollie.color(colors[i])
        draw_special(ollie, 100, 10, gap[i])
